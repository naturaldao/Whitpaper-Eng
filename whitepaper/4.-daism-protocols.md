...
* DAO Hardcore Protocol (Decentralized Autonomous Organism Hardcore Protocol)
* Unit Token Protocol
...
* If a DAO has issued its circulation token and sells its products or services in the circulation token (the unit is vita), when customers buy the DAO's products or services they can pay whatever tokens but the paid tokens will be converted by the IADD network to the circulation token and sent to the DAO's SDAF address. If a DAO doesn't issue its circulation token, it can only sell its products or services in uToken.
* Earnings made by a DAO are directly sent to its SDAF address. This ensures a DAO's long term development and a sustainable SDAF.
...
## Protocol of Decentralized Exchange and Decentralized Payment
