...
* 道易程的代币发行协议，会将每个DAO的所有代币，都直接发行到 IADD 网络。
* 和Uniswap类似，我们采用改进的CPMM算法为所有ERC20通证提供去中心化的定价方案。但和Uniswap以及班科不同的是，我们不会采用币价本身无法稳定的ETH或BNT来作为交易对的标配币种，我们用一种定价通证uToken取而代之。Like Uniswap we use a similar but improved CPMM algorithm to determine a real-time decentralized trading price for any ERC20 token that is listed and traded in the exchange. Unlike Uniswap or Bancor we use exactly the same volume of issued token as the base, and we don't use ETH or BNT whose prices in USD fluctuate hugely to form a trading pair but use a Unit Token(uToken) instead.

&#x20;

**IADD 网络**完全通过智能合约标准和协议，在无需项目方和土豪（而且他们也没可能）参与，特别是在不需要项目方投入人力和财力的情况下，在项目代币发行结束的那一刻，即自动构建好其通证的去中心化交易所！ IADD是一个通用的去中心化交易网络。

**IADD 网络**还通过去中心化去审核的机制，产生了一种在区块链去中心化市场的价格不可能发生变化的真正的定价通证uToken，该稳定币将成为区块链应用落地至关重要的基础。

**IADD 网络**部署好去中心化交易网络和定价通证uToken之后，就能完美地被应用到区块链一切支付需求中， **IADD网络**将成为以太坊一统天下的支付网络！
...
**We propose a real DEX Protocol and introduce a real stable coin uToken to solve these aforementioned issues**. Our solution doesn't need any party especially a token issuer to spend money and resources to bolster trading, while bearing a huge risk of losing money. In our solution right after the very first token's issue completes , a decentralized exchange will be initiated by IADD and a trading pair between the token and IADD will be listed. You can then buy or sell the token in the exchange. As more tokens are listed in the exchange you will be able to buy or sell any tokens listed in the exchange. IADD is a Uni-exchange Network.

&#x20;

The Unit Token, uToken, is issued by IADD utilizing a decentralized and decensored mechanism. uToken's price on IADD or decentralized markets is absolutely stable. Therefore adoption of uToken has big potentials to turn blockchain technologies into real popular applications.
...
Improved CPMM——Constant Product Market Maker is a model of market maker and generates token pricing in a non-linear way. CPMM is used by Uniswap, and we have imporved it.在Uniswap里得到应用的恒定乘积做市商模型（CPMM）是一种非线性币币兑换算法。我们进行里改良。

ISC—— IADD Smart Contract，我们部署的去中心化交易智能合约，它使用和班科网络类似的技术手段，为所有ERC20通证自动构建出其去中心化交易协议。It is a set of smart contracts which work similar to Bancor and establish a decentralized exchange protocol for ERC20 tokens.

IEP—— IADD Exchange Protocol。一个去中心化交易协议。It is a decentralized exchange protocol.

Unit Token, uToken——ISC智能合约部署的定价通证。

It is a Unit Token issued by ISC.

IADD Network(Instantly and Automatically Deployed DeSwap Network)——即刻自动部署的去中心化兑换网络，它是一个去中心化交易和去中心化支付2合1网络。a decentralized swap network that is instantly and automatically deployed.
...
3. 去中心化交易协议（Dex Protocol）\
   流通用通证（代币）全部发行到 IADD 网络后成为储备币。储备币通过自动计算而实现价值发现，并为市场提供流动性，且不必考虑交易规模大小。\
...
## 定价通证协议 （ETH FORGING model,, Issurance of uToken）

参见：[《定价通证协议 (Unit Token Protocol)》](https://shimo.im/docs/LKrkEVzDZNXUKAJp)
