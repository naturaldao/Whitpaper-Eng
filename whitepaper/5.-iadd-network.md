...
Like its name, Instantly and Automatically Deployed Devaluation Network, IADD Network is a decentralized exchange network that is instantly and automatically deployed.  It serves for the decentralized valuation of dApps, exchange and universal payment (settlement).
...
2.  Algorithm( Imporved Constant Product) provides forging rewards
    ...
    Reward Formula $$R = Δy_{n} = y_{n-1} - y_{n}= (k \div x_{0}) - (k \div x_{1}) = k \times(1/x_{0}-1/x_{1})$$\
    Where:\
