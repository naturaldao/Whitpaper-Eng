...
### Why do we need an ETH's variant?

* ETH has lost the chance to be a price prototype. Its price has been **abducted by the** legal tender of USA**.**
* **The issuance strategy of ETH is not suitable for the market's** burst, which needs a huge increase in a short time.
* Ether (ETH) is the native cryptocurrency to the Ethereum blockchain, it is not a token issued by any smart contract, which also means it does fit any ERC-20 or any other token standards. So, nomally, we have to minted it to WETH, which is an ERC-20 token. If we need an variant, then forge is the best strategy.
* In DAism's tokenomics, as a price prototype, uToken's price is solid. So it is a perfect token for investment's settlement or daily payments(just like you use fiat money everyday).
* uToken couldn't be "printed", it needs a value source. Also it must be issued by a decentralized way.
