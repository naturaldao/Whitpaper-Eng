...
* This protocol also includes an important rule for the upgrade of smart commons:  its dApp must be reliable, which means its dApp can not be modified by anyone once deployed.\
  ...
  Technically, Proxy Pattern is a suicide way for almost any smart common. If you find there's a bug in your dApp,  fix it and deploy it again to Ethereum and then, go back to DAism to register it as a new version of your smart common. So, Data Separation might be the best strategy for your development, and Contract Migration might be workable as well.
